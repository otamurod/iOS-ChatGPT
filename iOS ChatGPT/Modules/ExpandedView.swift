    ...
    var body: some View {
        NavigationStack {
            VStack {
                ...
                ScrollView {
                    VStack(spacing: 34) {
                        if model.isExpandedTextScreenEmpty, !model.expandedEntryText.isEmpty {
                            Text("Tap 'Expand' on the Top-Right corner of the screen")
                                .font(.system(size: 18))
                        ...
                        if model.isThinking {
                            VStack {
                                Text("Expanding the text...")
                                ProgressView().progressViewStyle(.circular)
                            }
                        }

                        if model.hasResultExpandedTextScreen {
                            ResultView(generatedText: model.generatedExpandedText).task {
...
struct ExpandedView_Previews: PreviewProvider {
    static var previews: some View {
        ExpandedView().environmentObject(AppModel())
    }
}
